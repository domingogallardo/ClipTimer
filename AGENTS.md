...
- Confirm the build intended for submission does not include beacon-only changes.
- Verify version and build numbers before archiving.
- Run the macOS test command before submitting:
  `xcodebuild test -scheme ClipTimer -project ClipTimer.xcodeproj -destination 'platform=macOS' -derivedDataPath ./DerivedData CODE_SIGNING_ALLOWED=NO`
- Archive and submit only from the public App Store configuration.
- If you need beacon presence for personal use, build `beacon` locally and keep it out of App Store submissions.
